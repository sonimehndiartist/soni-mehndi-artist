document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateWhatsappVisibility() {
        const isCartOpen = cartSlider && cartSlider.classList.contains('open');
        const isAddressOpen = addressSlider && addressSlider.classList.contains('open');
    ...
    // Placeholder UI update function for state synchronization
    ...
    authSubmitBtn.addEventListener('click', () => {
        ...
        if (mobile === '123456' && pass === '123456') {
            ...
            if (pendingRedirectToAddress) {
                ...
                if(addressSlider) addressSlider.classList.add('open');
    ...
    registerSubmitBtn.addEventListener('click', () => {
     ...
     // Process order upon pressing "Deliver Here"
    addressCardsList.addEventListener('click', (e) => {
        if (e.target.classList.contains('deliver-here-btn')) {
            const addressId = parseInt(e.target.getAttribute('data-id'));
            
            let storedAddresses = JSON.parse(localStorage.getItem('addressesList')) || [];
            const activeAddress = storedAddresses.find(a => a.id === addressId);
            ...
            localStorage.setItem('selectedAddress', JSON.stringify(activeAddress));

            const orderForm = {
                orderId: '#4' + Math.floor(Math.random() * 10000000).toString().padStart(7, '0'),
                user: activeUser,
                items: currentCart,
                deliveryDetails: activeAddress,
                paymentMethod: 'Pay Later / COD',
                timestamp: new Date().toISOString()
            };

            if (confirmAddrPerson) confirmAddrPerson.textContent = orderForm.deliveryDetails.contactPerson;
            if (confirmAddrText) confirmAddrText.textContent = orderForm.deliveryDetails.fullAddr;
            if (confirmAddrContact) confirmAddrContact.textContent = `Mobile: ${orderForm.deliveryDetails.mobile}`;
            if (confirmTxnDisplay) confirmTxnDisplay.textContent = `Payment Status: ${orderForm.paymentMethod}`;
            
            let totalValue = currentCart.reduce((sum, item) => sum + (item.price * item.qty), 0);
            totalValue += SHIPPING_CHARGE; 
            if (confirmTotalVal) confirmTotalVal.textContent = totalValue;

            if (addressSlider) addressSlider.classList.remove('open');
            ...
            if (typeof triggerPartyConfetti === 'function') {
